...
def affine_forward(x, w, b):
    ...
    Inputs:
    - x: A numpy array containing input data, of shape (N, d_1, ..., d_k)  #x是N行D列的
    - w: A numpy array of weights, of shape (D, M)  #w是D行M列的→只能用x乘w，不能反过来
    ...
    out = None
    ###########################################################################
    # TODO: Implement the affine forward pass. Store the result in out. You   #将结果存储到out
    # will need to reshape the input into rows.                       
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

    x_vector = x.reshape(x.shape[0], -1)  #X.reshape(X.shape[0], -1).T可以将一个维度为(a,b,c,d)的矩阵转换为一个维度为(b∗c∗d, a)的矩阵
